...
class GeniusComputerPlayer(Player):
    ...
    def minimax(self, state, player):
        ...
        if state.current_winner == other_player:
            # we should return position And score beacause we need to keep track of the score
            # for minimax to work
            return {
                'position' : None,
                'score' : 1 * (state.num_empty_squares() + 1) if other_player == max_player else -1 * (state.num_empty_squares() + 1)
            }
        elif not state.num_empty_squares(): # not empty squares
        ...
        for possible_move in state.available_moves():
            ...
            sim_score['position'] = possible_move # other this will get messed up from the recursion
